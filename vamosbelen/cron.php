...
class Connection
{
    ...
    static public function connect()
    {
        ...
        try {
            ...
            $link = new PDO("mysql:host=localhost;dbname=vamosbelen", "root", "");
...
foreach ($tables as $key => $value) {
    ...
    for ($i = 0; $i < count($response); $i++) {
        if (isset($response[$i]["id_" . $value])) {
            $delete = Controller::deleteData($value . "s", $response[$i]["id_" . $value], "id_" . $value);
        }
    }
}
